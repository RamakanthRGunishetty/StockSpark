...
df = spark \
    ...
    .option("subscribe", "ramakanth_stream") \
    .load()
...
class AggInsertTimeDB:
    def process(self, row):
        ...
        try:
            ...
            sql_insert_query = """ INSERT INTO sample_demo (StartTime, EndTime, Bank_Name, Avg_OpenPrice, Avg_HighPrice, Avg_LowPrice, Avg_ClosePrice, Avg_Volume) VALUES ('%s, '%s, '%s, '%d', '%d, '%d', '%d', '%d')""" % (StartTime, EndTime, Bank_Name, Open, High, Low, Close, Volume)

            print("\nsql_insert_query ", sql_insert_query)
            result = cursor.execute(sql_insert_query)
            result.commit()
            print(cursor.rowcount, "Record inserted successfully into sample_demo table")
